...
func TestHandleSettingsFailedNormalizeDoesNotMutateProxyPools(t *testing.T) {
	cfg := &config.Config{
		Mode:           "pool",
		LogLevel:       "info",
		Listener:       config.ListenerConfig{Address: "0.0.0.0", Port: 2323, Protocol: config.InboundProtocolMixed},
	...
	body := bytes.NewBufferString(`{
		...
		"log_level":"info",
		"skip_cert_verify":false,
		"geoip":{"enabled":true,"listen":"","port":2323,"auto_update_enabled":false,"auto_update_interval":"24h"}
	}`)
	req := httptest.NewRequest(http.MethodPut, "/api/settings", body)
	rec := httptest.NewRecorder()
	...
	if cfg.ProxyPools[0].Listener.Address != "" {
		t.Fatalf("proxy pool address mutated to %q after failed settings update", cfg.ProxyPools[0].Listener.Address)
	}
}
