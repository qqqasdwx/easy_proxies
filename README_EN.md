...
Easy Proxies was originally based on [jasonwong1991/easy_proxies](https://github.com/jasonwong1991/easy_proxies). We appreciate the original author's foundation and open-source work.

As of **2026-05-11**, this repository is maintained as an independent project. The v2.0.0 release introduced breaking changes, including SQLite-only runtime persistence, removal of `config.yaml` / `nodes.txt`, a redesigned WebUI, and a new runtime settings model. These changes are no longer compatible with the upstream repository, so future upstream fixes will be evaluated and ported manually instead of merged directly.
